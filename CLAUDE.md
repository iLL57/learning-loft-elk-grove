...
## Pending TODOs
- [ ] **Deploy the contact form fix** (branch `claude/contact-form-email-issue-xddb1g`): apply migration `20260903000000_create_contact_submissions.sql` and deploy the new `submit-contact` Edge Function via the Supabase dashboard/CLI (this session had no Supabase access to do it directly), then do a real end-to-end test submission through the live contact page and confirm it lands in the admin Contact Messages tab and both emails (admin notification + sender acknowledgment) arrive — same verification pattern as prior launches. Root cause: `contact.html`'s form was always front-end only (fake "Sending…" delay, no fetch call) — it never sent data anywhere, which is why the owner's test and a parent's real inquiry both went nowhere.
