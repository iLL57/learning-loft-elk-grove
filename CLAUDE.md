...
- [ ] SignWell and Square integration (Phase 2/3 of the enrollment workflow) — owner has both accounts already; blocked on API credentials being handed off via `supabase secrets set` (not pasted in chat). See TODO comments in `supabase/functions/admin-submission-action/index.ts` for exactly where each plugs in.
- [ ] Owner wants to track SignWell document-signed status and Square payment-received status automatically (webhook-driven) once Phase 2/3 credentials are available — `enrolled_students.documents_signed`/`initial_payment_received` fields already exist and are manually toggleable in the admin UI as a stand-in
