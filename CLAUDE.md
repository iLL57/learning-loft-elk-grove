...
- [ ] SignWell + Square Phase 2/3 code is deployed and live but dormant — waiting only on credentials from the owner, handed off via `supabase secrets set` (not pasted in chat):
  - `SIGNWELL_API_KEY`, `SIGNWELL_TEMPLATE_ID` (a SignWell template built from the 3 enrollment PDFs), optional `SIGNWELL_TEST_MODE` (defaults to `true`/safe until explicitly set to `false`)
  - `SQUARE_ACCESS_TOKEN`, `SQUARE_LOCATION_ID`, `SQUARE_INITIAL_FEE_CENTS` (the initial enrollment fee, in cents)
  - `SQUARE_WEBHOOK_SIGNATURE_KEY`, `SQUARE_WEBHOOK_NOTIFICATION_URL` (must exactly match the URL registered in the Square dashboard), `SIGNWELL_WEBHOOK_ID` (from SignWell's Create Webhook response, used as the HMAC key — SignWell has no separate signing secret)
  - Owner also needs to register these two webhook URLs in each platform's dashboard: `https://otmuflfpogtpkzqrclmm.supabase.co/functions/v1/signwell-webhook` and `https://otmuflfpogtpkzqrclmm.supabase.co/functions/v1/square-webhook`
  - Once set, `admin-submission-action`'s `send_package` action automatically creates one SignWell document + one Square payment link per student and emails both links to the applicant; `signwell-webhook`/`square-webhook` (deployed with `--no-verify-jwt`, own HMAC verification) flip `enrolled_students.documents_signed`/`initial_payment_received` automatically on `document_completed` / `payment.updated` (status `COMPLETED`) events. Until credentials are set, `send_package` behaves exactly as it did in Phase 1 (manual toggles still work as a fallback).
