...
- [ ] Square Phase 3 still pending credentials from the owner, handed off via `supabase secrets set` (not pasted in chat): `SQUARE_ACCESS_TOKEN`, `SQUARE_LOCATION_ID`, `SQUARE_INITIAL_FEE_CENTS` (the initial enrollment fee, in cents), `SQUARE_WEBHOOK_SIGNATURE_KEY`, `SQUARE_WEBHOOK_NOTIFICATION_URL` (must exactly match the URL registered in the Square dashboard: `https://otmuflfpogtpkzqrclmm.supabase.co/functions/v1/square-webhook`). Once set, `send_package` will also create a Square payment link per student and `square-webhook` will flip `enrolled_students.initial_payment_received` automatically on `payment.updated` (status `COMPLETED`). Until then, payment status stays manual-toggle only.
